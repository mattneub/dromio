...
extension Array where Element == SubsonicSong {
    /// Given a song id, find that song in the list and extract a sequence from that song to the
    /// end of the playlist.
    /// - Parameter song: The song to start with. Should be in the current playlist.
    /// - Returns: The sequence (array) of songs.
    ///
    func buildSequence(startingWith songId: String) -> [SubsonicSong] {
        guard let start = firstIndex(where: { $0.id == songId }) else {
            return []
        }
