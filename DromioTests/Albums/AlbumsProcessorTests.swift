...
struct AlbumsProcessorTests {
    ...
    func state() {
        subject.state.albums.append(.init(id: "1", name: "Yoho", artist: "Artist", songCount: 30, song: nil))
        #expect(presenter.statePresented?.albums.first == .init(id: "1", name: "Yoho", artist: "Artist", songCount: 30, song: nil))
    }

    @Test("receive initialData: sends `getAlbumList` to request maker, sets state `albums`")
    func receiveInitialData() async {
        requestMaker.albumList = [.init(id: "1", name: "Yoho", artist: "Artist", songCount: 30, song: nil)]
